...
<template>
  <nav class="flex justify-center mt-20" aria-label="Breadcrumb">
   ...
   <form class="space-y-8 divide-y divide-gray-200 mx-40">
     <div class="space-y-8 divide-y divide-gray-200 p-20 mx-40">
       <div class="pt-8">
         <div>
           <h3 class="text-lg font-medium leading-6 text-gray-900">회원 탈퇴</h3>
           <p class="mt-1 text-sm text-gray-500">여기는 회원탈퇴 페이지 입니다.</p>
         </div>
     ...
     <div class="pt-5 p-20">
       <div class="flex justify-center">
         ...
         <button @click="deleteUser" type="submit" class="ml-3 inline-flex justify-center rounded-md border border-transparent bg-indigo-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
         >삭제</button>
       </div>
     </div>
   </form>
 ...
 import DeleteUserRequest from "@/service/request/DeleteUserRequest.js";
 ...
 export default {
   ...
   watch: {
    ...
    async deleteUser() {
      ...
      console.log(req);
      try {
        let res = await this.api.deleteUser(req);
        if (res.code === ResultCode.Success) {
          console.log(res);
